...
class _LifeOZExactMasterHomeState extends State<LifeOZExactMasterHome> {
  ...
  void _showDesigns() => showModalBottomSheet<void>(
    context: context,
                  ...
                  child: ClipRRect(
                    ...
                    child: Stack(
                      ...
                      children: [
                        ...
                        if (_reality == i)
                          Container(decoration: BoxDecoration(border: Border.all(color: Colors.cyanAccent, width: 3), borderRadius: BorderRadius.circular(16))),
                      ],
                    ),
                  ),
                ),
...
class _CoreSymbol extends StatelessWidget {
  final int index;
  const _CoreSymbol({required this.index});
  @override
  Widget build(BuildContext context) {
    const colors = [Color(0xFF54F58A), Color(0xFFB55CFF), Color(0xFFFFB83D), Color(0xFF42D9FF), Color(0xFFC66BFF)];
    return CustomPaint(painter: _CorePainter(colors[index]));
  }
}

class _CorePainter extends CustomPainter {
  final Color color;
  _CorePainter(this.color);
  @override
  void paint(Canvas c, Size s) {
    final center = Offset(s.width / 2, s.height / 2);
    final p = Paint()..color = color.withOpacity(.9)..style = PaintingStyle.stroke..strokeWidth = 2.4;
    final g = Paint()..color = color.withOpacity(.22)..maskFilter = const MaskFilter.blur(BlurStyle.normal, 14);
    c.drawCircle(center, 18, g);
    c.drawOval(Rect.fromCenter(center: center, width: 72, height: 25), p);
    c.save();
    c.translate(center.dx, center.dy);
    c.rotate(.95);
    c.drawOval(Rect.fromCenter(center: Offset.zero, width: 72, height: 25), p);
    c.restore();
    c.drawCircle(center, 4, Paint()..color = Colors.white);
  }
  @override
  bool shouldRepaint(covariant _CorePainter oldDelegate) => oldDelegate.color != color;
}

class _CosmicPainter extends CustomPainter {
  @override
  void paint(Canvas c, Size s) {
    final p = Paint()..color = Colors.white.withOpacity(.22);
    for (var i = 0; i < 80; i++) {
      final x = (i * 83.0) % s.width;
      final y = (i * 137.0) % s.height;
      c.drawCircle(Offset(x, y), i % 7 == 0 ? 1.5 : .8, p);
    }
    final ring = Paint()..color = const Color(0xFF1C8CA0).withOpacity(.16)..style = PaintingStyle.stroke..strokeWidth = 1;
    for (final r in [s.width * .32, s.width * .43, s.width * .54]) {
      c.drawOval(Rect.fromCenter(center: Offset(s.width / 2, s.height * .47), width: r * 2, height: r * .58), ring);
    }
  }
  @override
