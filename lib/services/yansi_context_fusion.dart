...
/// Local-first: it does not invent facts and does not execute actions.
...
class YansiContextFusion {
  ...
  List<Map<String,dynamic>> _readMerged(String primary,String legacy){final records=<Map<String,dynamic>>[..._read(primary)];final legacyRaw=prefs.getString(legacy);if(legacyRaw!=null&&legacyRaw.isNotEmpty){try{final decoded=jsonDecode(legacyRaw);if(decoded is List){for(final item in decoded){if(item is Map)records.add(Map<String,dynamic>.from(item));}}}catch(_){}}return records;}

  String _temporalSignal(DateTime now,int openTasks,int upcoming,double spend){
    final hour=now.hour;
    if(upcoming>0&&hour>=18)return 'evening_commitment_review';
    if(openTasks>0&&hour<12)return 'morning_productivity_window';
    if(spend>0&&now.weekday==DateTime.sunday)return 'weekly_money_review';
    if(openTasks>0)return 'active_task_window';
    if(upcoming>0)return 'upcoming_commitment_window';
    return 'ambient';
  }
