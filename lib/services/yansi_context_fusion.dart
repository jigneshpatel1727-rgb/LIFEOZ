...
class YansiContextSnapshot {
  ...
  final String focus;
  const YansiContextSnapshot({required this.createdAt,required this.openTasks,required this.upcomingReminders,required this.expenseRecords,required this.householdRecords,required this.goals,required this.diaryEntries,required this.recentSpend,required this.recentThemes,required this.focus});

  Map<String,dynamic> toJson()=>{'createdAt':createdAt.toIso8601String(),'openTasks':openTasks,'upcomingReminders':upcomingReminders,'expenseRecords':expenseRecords,'householdRecords':householdRecords,'goals':goals,'diaryEntries':diaryEntries,'recentSpend':recentSpend,'recentThemes':recentThemes,'focus':focus};
  String get summary=>'Open tasks: $openTasks. Upcoming reminders: $upcomingReminders. Recent spending: $recentSpend. Goals: $goals. Household records: $householdRecords. Current focus: $focus.';
}
...
class YansiContextFusion {
  ...
  Future<YansiContextSnapshot> build() async {
    ...
    final upcoming=reminders.where((e){final d=DateTime.tryParse('${e['dueDate']??e['date']??''}');return d!=null&&!d.isBefore(now)&&d.isBefore(now.add(const Duration(days:14)));}).length;
    double spend=0;
    for(final e in expenses){final d=DateTime.tryParse('${e['date']??''}');if(d!=null&&now.difference(d).inDays<=30)spend+=(e['amount'] as num?)?.toDouble()??0;}
    final themes=<String>[];
