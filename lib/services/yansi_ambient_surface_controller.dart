...
class YansiAmbientSurfaceState {
  ...
  const YansiAmbientSurfaceState({this.title,this.message,this.confidence=0,this.rankingScore,this.deliveryMode,this.deliveryReason,this.visible=false,this.needsConfirmation=false,this.ambientOnly=true,this.voiceEligible=false,this.signalKey});
...
class YansiAmbientSurfaceController {
  ...
  Future<YansiAmbientSurfaceState> refreshFromRuntime(YansiProactiveRuntime runtime,{bool quietMode=false,bool userActive=true,bool screenVisible=true,bool voiceAvailable=true,DateTime? now}) async {
    ...
    if(item==null)return const YansiAmbientSurfaceState();
    final message=item.reason;
    if(title==null||title.trim().isEmpty||message.trim().isEmpty)return const YansiAmbientSurfaceState();
  ...
  String? ambientVoiceText(YansiAmbientSurfaceState state) {if(!state.voiceEligible||state.message==null)return null;final text=state.message!.trim();if(text.isEmpty)return null;return state.needsConfirmation?'I noticed something important. $text Please confirm before I act.':text;}
}
