...
class _LifeOSState extends State<LifeOS> with WidgetsBindingObserver {
  ...
  Future<void> _changeTheme(int index) async {
    final p = _profile;
    if (p == null) return;
    await _profiles.save(p.copyWith(themeIndex: index));
    if (mounted) setState(() => _profile = _profiles.load());
  }
  ...
  Widget build(BuildContext context) {
    if (_loading) {
      return const MaterialApp(
        ...
        home: Scaffold(
          backgroundColor: Color(0xFF01050A),
    ...
    return MaterialApp(
      ...
      theme: ThemeData(
        ...
        useMaterial3: true,
        scaffoldBackgroundColor: const Color(0xFF01050A),
      ),
      home: HyperFuturisticHomeScreen(
        prefs: widget.prefs,
