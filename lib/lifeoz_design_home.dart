...
class _LifeOZDesignHomeState extends State<LifeOZDesignHome> with SingleTickerProviderStateMixin {
  ...
  static const coreNames = [
    'Financial Intelligence', 'Goals & Growth', 'Productivity',
    'Household Management', 'Personal Life & Wellness'
  ];
          ...
          child: Column(
            children: [
              const Padding(
                ...
                child: Text('SIX REALITIES — TAP TO ACTIVATE', style: TextStyle(color: Colors.white, letterSpacing: 2, fontSize: 13)),
              ),
              Expanded(
                    ...
                    child: Stack(
                      ...
                      children: [
                        ClipRRect(
                          ...
                          child: Image.asset(realities[i].$1, fit: BoxFit.cover),
                        ),
                        Positioned(
                          bottom: 8,
                          left: 8,
                          ...
                          child: Text(
                            ...
                            style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold, letterSpacing: 1.4),
                          ),
                        ),
                      ],
                    ),
              ...
              child: Stack(
                children: [
                  Positioned(
                    ...
                    child: Row(
                      children: [
                        Image.asset('01_LifeOZ_App_Icon.png', width: 44, height: 44),
                        const SizedBox(width: 10),
                        const Text('LIFEOZ', style: TextStyle(color: Colors.white, fontSize: 25, letterSpacing: 7, fontWeight: FontWeight.w300)),
                      ],
                    ),
                  ),
                  Positioned(
                    ...
                    child: GestureDetector(
                      ...
                      child: Container(
                        ...
                        child: const Icon(Icons.auto_awesome, color: Color(0xFFFFB74D)),
                      ),
                    ),
                  ),
                  Center(
                      ...
                      child: AnimatedScale(
                        ...
                        child: Container(
                          ...
                          decoration: BoxDecoration(
                            ...
                            boxShadow: [BoxShadow(color: const Color(0xFF00BFFF).withOpacity(0.35), blurRadius: 55, spreadRadius: 12)],
                          ),
                  ...
                  _coreNode(0, 0.20, 0.35, const Color(0xFFFFB52E), Icons.account_balance_wallet_rounded),
                  _coreNode(1, 0.80, 0.35, const Color(0xFF58FF8B), Icons.track_changes_rounded),
                  _coreNode(2, 0.18, 0.67, const Color(0xFF3CD7FF), Icons.bolt_rounded),
                  _coreNode(3, 0.82, 0.67, const Color(0xFFB76CFF), Icons.home_work_rounded),
                  _coreNode(4, 0.50, 0.84, const Color(0xFFFF6F91), Icons.favorite_rounded),
                  Positioned(
                    bottom: 22,
                    left: 0,
                    right: 0,
                    child: Column(
                      children: [
                        Text(
                          _activeCore == null ? 'ONE TAP  •  ONE SCREEN  •  ONE REPORT' : coreNames[_activeCore!],
                          textAlign: TextAlign.center,
                          style: const TextStyle(color: Colors.white70, letterSpacing: 2, fontSize: 11),
                        ),
                        const SizedBox(height: 6),
                        const Text('LIVING INTELLIGENCE', style: TextStyle(color: Colors.white38, letterSpacing: 5, fontSize: 10)),
                      ],
                    ),
                  ),
                ],
  ...
  Widget _coreNode(int index, double x, double y, Color color, IconData icon) {
    ...
    return Positioned(
      left: MediaQuery.sizeOf(context).width * x - 42,
      top: MediaQuery.sizeOf(context).height * y - 42,
      width: 84,
      height: 84,
      child: GestureDetector(
        onTap: () => _core(index),
        child: AnimatedContainer(
          ...
          decoration: BoxDecoration(
            ...
            border: Border.all(color: color, width: active ? 3 : 1.6),
            boxShadow: [BoxShadow(color: color.withOpacity(active ? 0.55 : 0.25), blurRadius: active ? 30 : 16, spreadRadius: active ? 4 : 1)],
          ),
          child: Stack(
            alignment: Alignment.center,
            children: [
              Icon(icon, color: color, size: 32),
              Positioned.fill(child: CustomPaint(painter: _OrbitalPainter(color, _motion.value))),
            ],
          ),
        ),
      ),
...
class _ConstellationPainter extends CustomPainter {
  final double t;
  final int design;
  _ConstellationPainter(this.t, this.design);

  @override
  void paint(Canvas canvas, Size size) {
    final bg = Paint()
      ..shader = RadialGradient(colors: const [Color(0xFF071B42), Color(0xFF01040B)], radius: 0.85).createShader(Offset.zero & size);
    canvas.drawRect(Offset.zero & size, bg);
    final center = Offset(size.width / 2, size.height * 0.49);
    final orbit = Paint()..strokeWidth = 1..style = PaintingStyle.stroke..color = const Color(0xFF20BFFF).withOpacity(0.10);
    for (var i = 0; i < 3; i++) {
      canvas.drawOval(Rect.fromCenter(center: center, width: size.width * (0.65 + i * 0.14), height: size.height * (0.18 + i * 0.06)), orbit);
    }
    final stars = Paint()..style = PaintingStyle.fill;
    for (var i = 0; i < 70; i++) {
      final x = (math.sin(i * 12.9898 + design) * 0.5 + 0.5) * size.width;
      final y = (math.sin(i * 78.233 + design * 3) * 0.5 + 0.5) * size.height;
      stars.color = Colors.white.withOpacity(0.12 + 0.12 * ((i % 5) / 5));
      canvas.drawCircle(Offset(x, y), 0.7 + (i % 3) * 0.35, stars);
    }
  }

  @override
  bool shouldRepaint(covariant _ConstellationPainter old) => old.t != t || old.design != design;
}

class _OrbitalPainter extends CustomPainter {
  final Color color;
  final double t;
  _OrbitalPainter(this.color, this.t);

  @override
  void paint(Canvas canvas, Size size) {
    final paint = Paint()..color = color.withOpacity(0.22)..style = PaintingStyle.stroke..strokeWidth = 1.2;
    final radius = size.width * 0.38;
    for (var i = 0; i < 2; i++) {
      canvas.save();
      canvas.translate(size.width / 2, size.height / 2);
      canvas.rotate(t * math.pi * 2 + i * math.pi / 2);
  ...
  @override
  bool shouldRepaint(covariant _OrbitalPainter old) => old.t != t || old.color != color;
}
