...
public final class MainActivity extends Activity {
    ...
    final class AllinmydayView extends View {
        ...
        AllinmydayView(Context c) { super(c); start=System.currentTimeMillis(); text.setTypeface(android.graphics.Typeface.create("sans",0)); setBackgroundColor(Color.rgb(248,250,247)); }

        @Override protected void onDraw(Canvas c) {
            float w=getWidth(),h=getHeight(),cx=w*.5f,cy=h*.48f,t=(System.currentTimeMillis()-start)/1000f;
            drawBackground(c,w,h);
            for(int i=0;i<cores.length;i++) { Point q=project(cores[i][0],cores[i][1],cores[i][2],cx,cy,w,h), y=project(0,0,.12f,cx,cy,w,h); paint.setStyle(Paint.Style.STROKE); paint.setStrokeWidth(1); paint.setColor(Color.argb(55,75,110,85)); c.drawLine(y.x,y.y,q.x,q.y,paint); paint.setStyle(Paint.Style.FILL); }
            Point center=project(0,0,.12f,cx,cy,w,h); drawOrb(c,center.x,center.y,Math.min(w,h)*(.105f+.0025f*(float)Math.sin(t*1.7)));
            drawText(c,"iamyansi",center.x,center.y+5,17,Color.rgb(38,66,50),Paint.Align.CENTER);
            for(int i=0;i<cores.length;i++){ Point q=project(cores[i][0],cores[i][1],cores[i][2],cx,cy,w,h); drawCore(c,q.x,q.y,Math.min(w,h)*.040f,coreColors[i]); drawText(c,labels[i],q.x,q.y+Math.min(w,h)*.070f,11,Color.rgb(62,72,66),Paint.Align.CENTER); }
        ...
        @Override public boolean onTouchEvent(MotionEvent e){ switch(e.getActionMasked()){case MotionEvent.ACTION_DOWN:dragging=true;lastX=e.getX();lastY=e.getY();return true;case MotionEvent.ACTION_MOVE:if(dragging){yaw+=(e.getX()-lastX)*.004f;pitch+=(e.getY()-lastY)*.003f;pitch=Math.max(-.55f,Math.min(.55f,pitch));lastX=e.getX();lastY=e.getY();invalidate();}return true;case MotionEvent.ACTION_UP:dragging=false;performClick();return true;}return true; }
