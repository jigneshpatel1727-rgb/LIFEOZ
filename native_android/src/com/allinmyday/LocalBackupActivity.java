...
/** Backup is explicitly user initiated and handed to Android's standard save/share chooser. */
public final class LocalBackupActivity extends Activity {
    ...
    @Override public void onCreate(Bundle state) {
        ...
        generate.setOnClickListener(v -> { backupText = LocalBackup.export(store); preview.setText(backupText); }); root.addView(generate, new LinearLayout.LayoutParams(-1,58) {{ topMargin=22; }});
        Button save = new Button(this); save.setText("Save / Share backup"); save.setAllCaps(false); save.setTextColor(green); save.setEnabled(false);
        save.setOnClickListener(v -> startActivity(ShareBackup.create(backupText))); root.addView(save, new LinearLayout.LayoutParams(-1,52) {{ topMargin=8; }});
        ...
        generate.setOnClickListener(v -> { backupText = LocalBackup.export(store); preview.setText(backupText); save.setEnabled(true); });
        setContentView(root);
    }
}
